...
contract MToken is MTokenInterface, Exponential, TokenErrorReporter {
    ...
    function getAccountSnapshot(address account) external view returns (uint256, uint256, uint256, uint256) {
        ...
        return (uint256(Error.NO_ERROR), mTokenBalance, borrowBalance, exchangeRateMantissa);
    }
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowInternal(address borrower, uint256 repayAmount, MTokenInterface mTokenCollateral)
        internal
        nonReentrant
        returns (uint256, uint256)
    {
    ...
    function liquidateBorrowFresh(
        ...
        uint256 allowed = comptroller.liquidateBorrowAllowed(
            address(this), address(mTokenCollateral), liquidator, borrower, repayAmount
        );
        if (allowed != 0) {
        ...
        (uint256 amountSeizeError, uint256 seizeTokens) =
            comptroller.liquidateCalculateSeizeTokens(address(this), address(mTokenCollateral), actualRepayAmount);
