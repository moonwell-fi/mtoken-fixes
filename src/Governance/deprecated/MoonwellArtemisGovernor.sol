...
contract MoonwellArtemisGovernor {
    ...
    uint256 public quorumVotes = 100000000e18; // 100,000,000 WELL

    /// @notice The number of votes required in order for a voter to become a proposer
    uint256 public proposalThreshold = 400000e18; // 400,000 WELL

    /// @notice The maximum number of actions that can be included in a proposal
    uint256 public proposalMaxOperations = 25; // 25 actions
    ...
    function propose(
        ...
        require(
            _getVotingPower(msg.sender, sub256(block.number, 1)) > proposalThreshold,
        ...
        require(targets.length != 0, "GovernorArtemis::propose: must provide actions");
        require(targets.length <= proposalMaxOperations, "GovernorArtemis::propose: too many actions");
        ...
        uint256 endTimestamp = add256(block.timestamp, add256(votingPeriod, votingDelay));
    ...
    function cancel(uint256 proposalId) public {
        ...
        require(proposalState != ProposalState.Executed, "GovernorArtemis::cancel: cannot cancel executed proposal");

        Proposal storage proposal = proposals[proposalId];
        require(
            _getVotingPower(proposal.proposer, sub256(block.number, 1)) < proposalThreshold,
    ...
    function state(uint256 proposalId) public view returns (ProposalState) {
        require(proposalCount >= proposalId && proposalId > 0, "GovernorArtemis::state: invalid proposal id");
        Proposal storage proposal = proposals[proposalId];
    ...
    function castVote(uint256 proposalId, uint8 voteValue) public {
        return _castVote(msg.sender, proposalId, voteValue);
    }

    function castVoteBySig(uint256 proposalId, uint8 voteValue, uint8 v, bytes32 r, bytes32 s) public {
        bytes32 domainSeparator =
            keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes(name)), getChainId(), address(this)));
        bytes32 structHash = keccak256(abi.encode(BALLOT_TYPEHASH, proposalId, voteValue));
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ecrecover(digest, v, r, s);
        require(signatory != address(0), "GovernorArtemis::castVoteBySig: invalid signature");
    ...
    function _castVote(address voter, uint256 proposalId, uint8 voteValue) internal {
        require(state(proposalId) == ProposalState.Active, "GovernorArtemis::_castVote: voting is closed");
        Proposal storage proposal = proposals[proposalId];
        ...
        require(receipt.hasVoted == false, "GovernorArtemis::_castVote: voter already voted");
    ...
    function _getVotingPower(address voter, uint256 blockNumber) internal view returns (uint256) {
        ...
        uint256 distibutorVotes = distributor.getPriorVotes(voter, blockNumber);
        uint256 safetyModuleVotes = safetyModule.getPriorVotes(voter, blockNumber);

        return add256(add256(uint256(wellVotes), distibutorVotes), safetyModuleVotes);
    }
    ...
    function sweepTokens(address tokenAddress, address destinationAddress) external {
        require(msg.sender == address(timelock), "GovernorArtemis::sweepTokens: sender must be timelock");
    ...
    function setQuorumVotes(uint256 newValue) external {
    ...
    function setProposalThreshold(uint256 newValue) external {
        require(msg.sender == address(timelock), "only timelock");
    ...
    function setVotingDelay(uint256 newValue) external {
        require(msg.sender == address(timelock), "only timelock");
    ...
    function setProposalMaxOperations(uint256 newValue) external {
        require(msg.sender == address(timelock), "only timelock");
    ...
    function setVotingPeriod(uint256 newValue) external {
        require(msg.sender == address(timelock), "only timelock");
    ...
    function __executeBreakGlassOnCompound(CompoundSetPendingAdminInterface[] calldata addresses) external {
        require(msg.sender == breakGlassGuardian, "GovernorArtemis::__breakglass: sender must be bg guardian");

        uint256 length = addresses.length;
        for (uint256 i = 0; i < length; ++i) {
            timelock.fastTrackExecuteTransaction(
                address(addresses[i]), 0, "_setPendingAdmin(address)", abi.encode(governanceReturnAddress)
            );
        }
    }
    ...
    function __executeBreakGlassOnSetAdmin(SetAdminInterface[] calldata addresses) external {
        ...
        for (uint256 i = 0; i < length; ++i) {
            timelock.fastTrackExecuteTransaction(
                address(addresses[i]), 0, "setAdmin(address)", abi.encode(governanceReturnAddress)
            );
        }
    }
    ...
    function __executeBreakGlassOnSetPendingAdmin(SetPendingAdminInterface[] calldata addresses) external {
        ...
        for (uint256 i = 0; i < length; ++i) {
            timelock.fastTrackExecuteTransaction(
                address(addresses[i]), 0, "setPendingAdmin(address)", abi.encode(governanceReturnAddress)
            );
        }
    }
    ...
    function __executeBreakGlassOnChangeAdmin(ChangeAdminInterface[] calldata addresses) external {
        ...
        for (uint256 i = 0; i < length; ++i) {
            timelock.fastTrackExecuteTransaction(
                address(addresses[i]), 0, "changeAdmin(address)", abi.encode(governanceReturnAddress)
            );
        }
    }
    ...
    function __executeBreakGlassOnOwnable(OwnableInterface[] calldata addresses) external {
        ...
        for (uint256 i = 0; i < length; ++i) {
            timelock.fastTrackExecuteTransaction(
                address(addresses[i]), 0, "transferOwnership(address)", abi.encode(governanceReturnAddress)
            );
        }
    }
    ...
    function __executeBreakGlassOnEmissionsManager(SetEmissionsManagerInterface[] calldata addresses) external {
        ...
        for (uint256 i = 0; i < length; ++i) {
            timelock.fastTrackExecuteTransaction(
                address(addresses[i]), 0, "setEmissionsManager(address)", abi.encode(governanceReturnAddress)
            );
        }
    }
    ...
    function __acceptAdminOnTimelock() external {
        require(msg.sender == breakGlassGuardian, "GovernorArtemis::__acceptAdmin: sender must be bg guardian");
    ...
    function __removeGuardians() external {
        ...
        require(msg.sender == address(timelock), "GovernorArtemis::__removeGuardians: sender must be the timelock");

        // Removing the governance guardian can only occur after the sunset.
        require(block.timestamp >= guardianSunset, "GovernorArtemis::__removeGuardians cannot remove before sunset");
...
interface TimelockInterface {
    ...
    function fastTrackExecuteTransaction(address target, uint256 value, string calldata signature, bytes calldata data)
        external
        payable
